...
public class GUI extends JFrame implements ActionListener
{
    ...
    public void actionPerformed(ActionEvent click)  
    {
            ...
            for(int colMove = 0; colMove < square[0].length; colMove++)
            {
                if(source == square[rowMove][colMove] && movesMade < maxMoves)  
                {
                       if (board[rowMove][colMove] == "") {
                            ...
                            board[rowMove][colMove] = startingPlayer;
                                if (Logic.ifWinRows(startingPlayer, board, 3, rowMove, colMove) 
                                     || Logic.ifWinCols(startingPlayer, board, 3, rowMove, colMove)) {
                                    endofgame = true;
                                }
                
                                if (rowMove == 0 || rowMove == 2) {
                                    if (Logic.ifWinDiag(startingPlayer, board, 3, rowMove, colMove)) {
                                        endofgame = true;
                                    }
                                }
                                ...
                                else startingPlayer = "X";
                                panelPlayingField.requestFocus();
                                movesMade++;
                            }
                       else JOptionPane.showMessageDialog(null, "Please Select a New Box", 
                    "Already Chosen", JOptionPane.ERROR_MESSAGE);
                }
            }
        ...
        if (endofgame) {
            ...
            int winTrue = JOptionPane.showConfirmDialog(null, startingPlayer + " wins," +
                            " Do you want to play again?"
                        , "New Game?" ,JOptionPane.YES_NO_OPTION);
                    if(winTrue == JOptionPane.YES_OPTION)    
                    {
                        inGame = false;
                        startingPlayer = "";
        ...
        if (maxMoves == movesMade) {
            int gameover = JOptionPane.showConfirmDialog(null, "No moves left," +
                            " do you want to play again?"
                        , "New Game?" ,JOptionPane.YES_NO_OPTION);
